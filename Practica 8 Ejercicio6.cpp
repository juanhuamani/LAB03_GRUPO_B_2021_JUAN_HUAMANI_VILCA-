...
class Forma {
	...
	public:
    	...
    	virtual void cambiar_color(string _color){ color=_color;};
    	void mover_centro();
...
class Elipse : public Forma{
	...
	public:
		Elipse(){};
		~Elipse(){};
		Elipse(string _color,string _nombre_forma){color=_color;nombre_forma=_nombre_forma;};
...
Circulo::Circulo(string _color,string _nombre_forma,float _radio_menor):Elipse(_color, _nombre_forma){
	radio_menor = _radio_menor;
}
...
int main(){
	...
	Rectangulo *rec;
	Forma *form = new Forma[4];
	form[0]=Rectangulo("azul","rectangulo",5,7);
	form[1]=Elipse("Rosa","Elipse",9,6);
	form[2]=Cuadrado("Blanco","Cuadrado",3);
	form[3]=Circulo("Verde","Circulo",5);
	
	
	for(int i=0;i<4;i++){
		form[i].cambiar_color("azul");
